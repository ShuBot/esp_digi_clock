...
uint8_t DM_INTENSITY        = 0;     // Dot-Matrix LED Intensity Range: 0-15
...
// Setup, sets display config parameters, gets time from NTP over wifi.
void setup(){
  Display.begin();
  Display.setIntensity(DM_INTENSITY);
  Display.displayText(curMessage, PA_CENTER, scrollSpeed, scrollPause, PA_PRINT, PA_NO_EFFECT);
  // Display.displayClear();

  if(SERIAL_DEBUG) {Serial.begin(115200);}
  // Connect to Wi-Fi
  if(SERIAL_DEBUG) {PRINT("Connecting to ");}
  if(SERIAL_DEBUG) {PRINTS(ssid);}
  WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    if(SERIAL_DEBUG) {PRINT(".");}
  }
  if(SERIAL_DEBUG) {PRINTS("");}
  if(SERIAL_DEBUG) {PRINTS("WiFi connected.");}
...
// Updates "curMessage" buffer with the right time data.
void printLocalTime(){
  struct tm timeinfo;
  if(!getLocalTime(&timeinfo)){
    if(SERIAL_DEBUG) {PRINTS("Failed to obtain time");}
    strcpy(curMessage, waitMessage);
  } else {
    if(SERIAL_DEBUG) {PRINTS("Local time obtained successfully");}
    if(SERIAL_DEBUG) {PRINTS(&timeinfo, "%A, %B %d %Y %H:%M:%S");}
